"""
Orthographic phonotactic validator plus CM-only sentence experiment export.
...
3. Optionally run a CM-only sentence reconstruction experiment over
   `SCRIPT_CONTIN` sentences, testing x = 10, 20, ..., 100.

The CM sentence mode does not use GR / LM / COH yet. It builds candidate word
splits from orthographic scores, keeps single-letter tokens for initials, scores
whole sentence candidates with a context-style sentence model, and exports the
input/output table to Excel.
"""
...
def search_all_candidates(
    ...
    final_states = completed or [state for state in beam if state.position >= n_chars]
    return list(dict.fromkeys(state.tokens for state in final_states))
...
def format_display_sentence(tokens: list[str] | tuple[str, ...]) -> str:
    return " ".join(token.upper() for token in tokens if token)
...
def run_cm_sentence_experiment(
    ...
    best_ws = workbook.create_sheet(title="cm best output")
    best_ws.append(
        [
            "sheet_name",
            "row_number",
    ...
    if not target_sheet_names:
        raise SystemExit("None of the requested CM evaluation sheets were found in the workbook.")
        ...
        for row_index, row in enumerate(df.itertuples(index=False), start=1):
            ...
            for x_percent in x_values:
                ...
                for tokens in candidate_token_sequences:
                    metrics = context_model.score_sentence(tokens)
                    scored_candidates.append(
                        {
                            "possible_sentence": format_display_sentence(tokens if isinstance(tokens, tuple) else tuple(tokens)),
                            "cm_score": float(metrics["cm_score"]),
                            "token_count": int(metrics["token_count"]),
                        }
                    )

                scored_candidates.sort(
                    key=lambda item: (-float(item["cm_score"]), int(item["token_count"]), str(item["possible_sentence"]))
                )
                ...
                for index, candidate in enumerate(scored_candidates):
                    detail_ws.append(
                        [
                            sheet_name if index == 0 else "",
                            row_index if index == 0 else "",
                            getattr(row, "ART_ID", "") if index == 0 else "",
                            getattr(row, "PARA_ID", "") if index == 0 else "",
                            getattr(row, "SENT_ID", "") if index == 0 else "",
                            input_sentence if index == 0 else "",
                            reference_sentence if index == 0 else "",
                            x_percent if index == 0 else "",
                            candidate["possible_sentence"],
                            candidate["cm_score"],
                            best_candidate["possible_sentence"] if index == 0 else "",
                        ]
                    )
                    detail_row_total += 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cm-eval-sheets", type=str, default="test")
...
def main() -> None:
    ...
    if args.cm_sentence_file is not None:
        ...
        run_cm_sentence_experiment(
            ...
            eval_sheets=[sheet.strip() for sheet in args.cm_eval_sheets.split(",") if sheet.strip()],
        )
        return
