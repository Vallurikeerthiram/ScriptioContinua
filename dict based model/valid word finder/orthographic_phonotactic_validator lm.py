...
class NgramLanguageModelScorer:
    def __init__(self, alpha: float = 0.2, backoff: float = 0.4) -> None:
        self.alpha = alpha
        self.backoff = backoff
        self.unigram_counts: Counter[str] = Counter()
        self.bigram_counts: Counter[tuple[str, str]] = Counter()
        self.trigram_counts: Counter[tuple[str, str, str]] = Counter()
        self.bigram_context_totals: Counter[tuple[str, str]] = Counter()
        self.unigram_context_totals: Counter[str] = Counter()
        self.total_tokens = 0
        self.vocab: set[str] = set()
        self.train_raw_mean = -8.0
        self.train_raw_std = 1.0
    ...
    def fit(self, token_sequences: list[list[str]]) -> None:
        ...
        self.unigram_context_totals = Counter()
        self.total_tokens = 0
        self.vocab = set()

        raw_values: list[float] = []
        for tokens in token_sequences:
            normalized_tokens = [normalize_word(token) for token in tokens if normalize_word(token)]
            if not normalized_tokens:
                continue

            self.vocab.update(normalized_tokens)
            chain = ["<s>", "<s>"] + normalized_tokens + ["</s>"]
            self.unigram_counts.update(chain[2:])
            self.total_tokens += len(chain) - 2

            for left, right in zip(chain[1:], chain[2:]):
                self.bigram_counts[(left, right)] += 1
                self.unigram_context_totals[left] += 1

            for first, second, third in zip(chain, chain[1:], chain[2:]):
                self.trigram_counts[(first, second, third)] += 1
                self.bigram_context_totals[(first, second)] += 1

            raw_values.append(self.raw_sentence_score(normalized_tokens))

        if raw_values:
            self.train_raw_mean = statistics.mean(raw_values)
            self.train_raw_std = statistics.pstdev(raw_values) or 1.0

    def _unigram_prob(self, token: str) -> float:
        vocab_size = max(1, len(self.vocab) + 1)
        return (self.unigram_counts.get(token, 0) + self.alpha) / (
            self.total_tokens + self.alpha * vocab_size
        )

    def _bigram_prob(self, previous: str, token: str) -> float:
        vocab_size = max(1, len(self.vocab) + 1)
        count = self.bigram_counts.get((previous, token), 0)
        total = self.unigram_context_totals.get(previous, 0)
        return (count + self.alpha) / (total + self.alpha * vocab_size)

    def _trigram_prob(self, prev2: str, prev1: str, token: str) -> float:
        vocab_size = max(1, len(self.vocab) + 1)
        count = self.trigram_counts.get((prev2, prev1, token), 0)
        total = self.bigram_context_totals.get((prev2, prev1), 0)
        return (count + self.alpha) / (total + self.alpha * vocab_size)

    def raw_sentence_score(self, tokens: list[str] | tuple[str, ...]) -> float:
        normalized_tokens = [normalize_word(token) for token in tokens if normalize_word(token)]
        if not normalized_tokens:
            return -20.0
        ...
        for index in range(2, len(chain)):
            prev2, prev1, token = chain[index - 2], chain[index - 1], chain[index]

            trigram_count = self.trigram_counts.get((prev2, prev1, token), 0)
            bigram_count = self.bigram_counts.get((prev1, token), 0)

            if trigram_count > 0:
                probability = self._trigram_prob(prev2, prev1, token)
            elif bigram_count > 0:
                probability = self.backoff * self._bigram_prob(prev1, token)
            else:
                probability = (self.backoff ** 2) * self._unigram_prob(token)

            log_sum += math.log(max(probability, 1e-12))
            steps += 1

        return log_sum / max(1, steps)
    ...
    def score_tokens(self, tokens: list[str] | tuple[str, ...]) -> dict[str, float | int | str]:
        ...
        raw_lm_score = self.raw_sentence_score(token_list)
        lm_score = logistic((raw_lm_score - self.train_raw_mean) / (self.train_raw_std * 4.0))
        return {
            "output_sentence": " ".join(token_list),
            "lm_score": lm_score,
            "raw_lm_score": raw_lm_score,
            "token_count": len(token_list),
        }
...
def run_lm_sentence_experiment(
    sentence_file: Path,
    output_file: Path,
    validator: PhonotacticWordlikenessValidator,
    x_values: list[int],
    ...
    sheets = load_sent_id_workbook(sentence_file)
    if "train" not in sheets:
        raise SystemExit("The LM experiment expects a 'train' sheet.")

    context_model = SentenceContextModel(validator)
    train_sequences = build_training_token_sequences(sheets["train"])
    context_model.fit(train_sequences)

    lm_scorer = NgramLanguageModelScorer()
    lm_scorer.fit(train_sequences)
    ...
    best_ws = workbook.create_sheet(title="lm best output")
    best_ws.append(best_headers)
    best_row_total = 0
    detail_row_total = 0
    detail_sheets = {x_percent: workbook.create_sheet(title=f"x={x_percent}") for x_percent in x_values}
    ...
    target_sheet_names = [sheet_name for sheet_name in eval_sheets if sheet_name in sheets]
    if not target_sheet_names:
        raise SystemExit("None of the requested LM evaluation sheets were found in the workbook.")
    ...
    for sheet_name in target_sheet_names:
        ...
        for row_index, row in enumerate(df.itertuples(index=False), start=1):
            ...
            for x_percent in x_values:
                ...
                scored_candidates = []
                for tokens in candidate_token_sequences:
                    metrics = lm_scorer.score_tokens(tokens)
                    scored_candidates.append(
                        {
                            "possible_sentence": format_display_sentence(tokens if isinstance(tokens, tuple) else tuple(tokens)),
                            "lm_score": float(metrics["lm_score"]),
                            "token_count": int(metrics["token_count"]),
                        }
                    )

                scored_candidates.sort(
                    key=lambda item: (-item["lm_score"], item["token_count"], item["possible_sentence"])
                )
    ...
    if best_row_total == 0:
        print("Warning: No results generated for LM experiment.")
        return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lm-sentence-file", type=Path, help="Run the LM-only sentence experiment.")
    parser.add_argument("--lm-output-file", type=Path, help="Output Excel path for the LM experiment.")
    parser.add_argument("--lm-eval-sheets", type=str, default="test")
...
def main() -> None:
    ...
    if args.lm_sentence_file is not None:
        output_file = args.lm_output_file
        if output_file is None:
            output_file = Path("results/spreadsheets/lm_sentence_results.xlsx")
        run_lm_sentence_experiment(
            sentence_file=args.lm_sentence_file,
            output_file=output_file,
            validator=validator,
            x_values=parse_x_values(args.cm_x_values),
            max_word_len=args.cm_max_word_len,
            beam_cap=args.cm_beam_cap,
            eval_sheets=[sheet.strip() for sheet in args.lm_eval_sheets.split(",") if sheet.strip()],
        )
        return
