"""
Generate per-model score plots from a labeled Excel sheet.

How this file works:
This script reads a dataset Excel file where:
- `word` contains the token
- `label` contains the ground-truth class
- every other numeric column is treated as a model output column

Plot behavior:
- `label = 1` is drawn in green
- `label = 0` is drawn in red
- repeated or nearly identical x-values are stacked vertically so overlapping
  points are still visible
- green points stack upward from zero and red points stack downward from zero
"""
...
DEFAULT_INPUT = Path("test dataset.xlsx")
DEFAULT_OUTPUT = Path("results/plots/test_dataset_model_graphs")
EXCLUDED_COLUMNS = {"word", "label"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create one graph per model column from a labeled Excel file.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Decimal precision used when stacking near-identical scores.",
    )
...
def build_stacked_points(df: pd.DataFrame, column: str, precision: int) -> pd.DataFrame:
    ...
    plot_df["stack_index"] = (
        plot_df.groupby(["label", "stack_key"]).cumcount() + 1
    )
...
def main() -> None:
    ...
    if "label" not in df.columns:
        raise SystemExit("The input file must contain a 'label' column.")

    model_columns = detect_model_columns(df)
    if not model_columns:
        raise SystemExit("No numeric model columns were found in the input file.")
