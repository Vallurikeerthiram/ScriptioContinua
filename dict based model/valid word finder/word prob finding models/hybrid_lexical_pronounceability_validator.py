...
SIBLING_VALIDATOR_DIR = Path(__file__).resolve().parents[1]
if str(SIBLING_VALIDATOR_DIR) not in sys.path:
    sys.path.insert(0, str(SIBLING_VALIDATOR_DIR))

from syllable_backoff_pronounceability_validator import PronounceabilityValidator


class HybridWordValidator:
    def __init__(self, lexicon: dict[str, dict[str, float | int]], pronounceability: PronounceabilityValidator) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Hybrid word validator: corpus lexicon first, pronounceability fallback.")
