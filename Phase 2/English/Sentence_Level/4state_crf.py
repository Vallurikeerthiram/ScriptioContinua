...
OUTPUT_PATH = Path("4state_crf_results.xlsx")
RANDOM_SEED = 42
PAD_TOKEN = "<PAD>"
UNK_TOKEN = "<UNK>"
PAD_LABEL = -100
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    required = ["ART_ID", "PARA_ID", "SENT_ID", "SENT_ORI", "SCRIPT_CONTIN", "STATE_2", "STATE_4"]
...
def load_split_data(excel_path: Path) -> Dict[str, pd.DataFrame]:
    ...
    missing_sheets = [sheet for sheet in ("train", "val", "test") if sheet not in sheet_lookup]
    if missing_sheets:
        raise ValueError(f"Workbook must contain train, val, and test sheets. Missing: {missing_sheets}")

    for split_name in ("train", "val", "test"):
        df = normalize_columns(workbook.parse(sheet_lookup[split_name]))
        ...
        if invalid_count:
            warnings.warn(
                f"{split_name}: dropped {invalid_count} invalid rows where script and label lengths differed."
            )
...
class SequenceTaggerBase(nn.Module):
    def __init__(
        ...
        num_labels: int,
        use_crf: bool = False,
        crf_kwargs: Dict[str, object] | None = None,
...
class CRFTagger(nn.Module):
    def __init__(
        ...
        self.crf = CRFLayer(num_labels, **(crf_kwargs or {}))
        self.invalid_penalty_masks = invalid_penalty_masks or {}
        self.invalid_penalty_weight = invalid_penalty_weight
    ...
    def decode(self, inputs: torch.Tensor, mask: torch.Tensor) -> List[List[int]]:
        return self.crf.decode(self.forward(inputs), mask)
...
def compute_invalid_bies_penalty(
    ...
    start_penalty = probabilities[:, 0, :][:, invalid_masks["start"]].sum(dim=1)
    ...
    end_penalty = last_probabilities[:, invalid_masks["end"]].sum(dim=1)
...
def run_experiment(config: Config) -> None:
    ...
    model_names = ["bilstm", "rnn", "cnn", "gru"]
    summary_frames: Dict[str, pd.DataFrame] = {}
    prediction_frames: Dict[str, pd.DataFrame] = {}
    confusion_frames: Dict[str, pd.DataFrame] = {}
    ...
    for model_name in model_names:
        ...
        model = build_model(model_name, embedding_weights, config, len(label_to_id), label_scheme, label_to_id).to(device)
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Train 4-state CRF sequence models on the sentence-based split dataset.")
    parser.add_argument("--excel-path", type=Path, default=EXCEL_PATH)
    parser.add_argument("--output-path", type=Path, default=OUTPUT_PATH)
    parser.add_argument("--epochs", type=int, default=Config.epochs)
    parser.add_argument("--batch-size", type=int, default=Config.batch_size)
    parser.add_argument("--require-gpu", action="store_true", default=Config.require_gpu)
    parser.add_argument("--allow-cpu", action="store_false", dest="require_gpu")
