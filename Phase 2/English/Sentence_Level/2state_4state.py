...
DEFAULT_EXCEL_PATH = Path("Sent_based_split.xlsx")
DEFAULT_OUTPUT_PATH = Path("plain_model_results.xlsx")
RANDOM_SEED = 42
PAD_TOKEN = "<PAD>"
UNK_TOKEN = "<UNK>"
PAD_LABEL = -100
...
def predict_with_model(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for idx in range(predicted_tensor.size(0)):
                true_seq = labels[idx][mask[idx]].tolist()
                pred_seq = predicted_tensor[idx][mask[idx]].tolist()
...
def run_experiment(config: Config) -> None:
    ...
    print("Embedding: trainable 64-dimensional character embedding shared across all plain models.")
    ...
    for label_scheme in ("state2", "state4"):
        ...
        for model_name, model_display_name in MODEL_DISPLAY_NAMES.items():
            print(f"\nTraining {model_display_name} on {label_scheme.upper()} ...")
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Train plain sequence models on the sentence-based split dataset.")
